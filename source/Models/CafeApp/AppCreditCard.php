...
class AppCreditCard extends Model
{
    ...
    {
        parent::__construct("app_credit_cards", ["id"], ["user_id", "brands", "last_digits", "cvv", "hash"]);
    ...
    public function creditCard(User $user, string $number, string $name, string $expDate, string $cvv): ?AppCreditCard
    {
        $this->build = [
            ...
            "card_holder_name" => $this->clear($name, FILTER_SANITIZE_FULL_SPECIAL_CHARS),
        ...
        if (empty($this->callback->id) || !$this->callback->valid) {
            $this->message->warning("não foi possivel validar o cartao");
            return null;
        }
